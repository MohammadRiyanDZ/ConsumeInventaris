...
export default function Inbound() {
    ...
    useEffect(() => {
        instance.get('inbound-stuff/data')
        .then(res => {
            console.log('API Response:', res.data.data);
            if (Array.isArray(res.data.data)) {
                setInbound(res.data.data);
            } else {
                setError({ message: 'Invalid data format from API.' });
            }
        })
        .catch(err => {
    ...
    const deleteInbound = (id) => {
        ...
        instance.delete(`inbound-stuff/${id}`)
        .then(() => {
                            ...
                            <tbody>
                                {Array.isArray(inbound) && inbound.map((item, index) => (
                                    <tr key={item.id} className="border-b dark:border-neutral-500">
                                        <td className="whitespace-nowrap text-black px-6 py-4">{index + 1}</td>
                                        <td className="whitespace-nowrap text-black px-6 py-4">{item.stuff_id}</td>
                                        <td className="whitespace-nowrap text-black px-6 py-4">{item.total}</td>
                                        <td className="whitespace-nowrap text-black px-6 py-4">{item.date}</td>
                                        <td className="whitespace-nowrap text-black px-6 py-4">
