    ...
    const deleteLending = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const token = localStorage.getItem('token');
                axios.delete(`http://localhost:8000/lending/delete/${id}`, {
                    headers: {
                        'Authorization': 'Bearer ' + token,
                    }
                })
                            ...
                            <tbody>
                                {lending.map((lending, index) => (
                                    <tr key={lending.id} className="border-b dark:border-neutral-500">
                                        ...
                                        <td className="text-black px-6 py-4">{lending.name}</td>
                                        <td className="text-black px-6 py-4">{lending.date_time}</td>
                                        <td className="text-black px-6 py-4">{lending.stuff_id}</td>
                                        <td className="text-black px-6 py-4">{lending.user_id}</td>
                                        <td className="text-black px-6 py-4">{lending.notes}</td>
                                        <td className="text-black px-6 py-4">{lending.total_stuff}</td>
                                        {authUser && authUser.role === 'Staff' && (
